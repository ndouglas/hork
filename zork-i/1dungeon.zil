...
<ROUTINE GO ()
  ...
  // QUEUE means to start an interrupt routine. Its arguments are the name of
  // the routine (e.g. "I-FIGHT") and the number of turns from now you wish it
  // to run (e.g. 10). In this case, we're telling it to run at -1 turns, which
  // means it will run continuously until we dequeue it.

  // General fighting routine.
	<ENABLE <QUEUE I-FIGHT -1>>

  // Start the sword routine, which causes the sword to glow when you are in
  // danger.
	<QUEUE I-SWORD -1>

  // Start the thief routine.
	<ENABLE <QUEUE I-THIEF -1>>

  // Once we've seen the candles, they have a limited lifespan.
	<QUEUE I-CANDLES 40>

  // The battery-powered brass lantern has a limited lifespan.
	<QUEUE I-LANTERN 200>

  // Set the "VTYPE" (which I believe means "vehicle type") property of the
  // inflated boat to travel on water. Or perhaps in the air too?
  ...
  // Set our current location to WEST-OF-HOUSE.
  ...
  // WINNER is usually the player, except when we are talking to a character.
  // In _Zork I_, we don't really have to worry much about this.
	<SETG WINNER ,ADVENTURER>

  // PLAYER is always the player object.
	<SETG PLAYER ,WINNER>

  // Move the PLAYER to WEST-OF-HOUSE.
	<MOVE ,WINNER ,HERE>
