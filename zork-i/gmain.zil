...
//
// Anyway, these constants are used in various different ways to control the
// game. I'll go through each in turn.

// M-FATAL is used to flush the inputs. For instance, if you type "s;e;s;w" but
// bump into a wall or your lantern dies or something after that first command,
// the subsequent inputs are flushed to give the player a chance to recover.

<CONSTANT M-FATAL 2>

// I don't see M-HANDLED mentioned anywhere else.

<CONSTANT M-HANDLED 1>

// Or M-NOT-HANDLED. Which might be as intended. Perhaps it's a default value?

<CONSTANT M-NOT-HANDLED <>>

// Or M-OBJECT, which might mean that it's used as a default.

<CONSTANT M-OBJECT <>>

// A room routine is called with M-BEG to give it a chance to act early in each
// game turn. For instance, if you attempt certain actions in a room that does
// not want to allow those actions, the game can intercept and prevent it.

<CONSTANT M-BEG 1>

// A room routine is called with M-END to give it a chance to act last in each
// game turn. This might add just a bit of descriptive text, for flavor, or it
// might blow up and kill the player.

<CONSTANT M-END 6>

// A room routine is called with M-ENTER to give the room a chance to act when
// the player enters. For instance, the Gas Room might check for an open flame,
// although this is not how it is actually implemented in practice.

<CONSTANT M-ENTER 2>

// A room routine is called with M-LOOK to show the room description when the
// player enters for the first time or when VERBOSE mode is enabled.

<CONSTANT M-LOOK 3>

// Not sure what M-FLASH does.

<CONSTANT M-FLASH 4>

// Not sure what M-OBJDESC does. I'm assuming it's an object description.

<CONSTANT M-OBJDESC 5>

;"GO now lives in SPECIAL.ZIL"

// This is the actual game loop, as opposed to the GO routine, which is their
// equivalent of `main()`.
