...
// I believe "buzz-words" are ignored by the compiler, but this is weirdly hard
// to confirm. _Learning ZIL_ doesn't seem to mention them, and `zil.txt` says
// only:
//
// BUZZ
...
// Ignore the "magic words" unless we're in _Zork II_, and a couple more unless
// we're in _Zork III_.
//
// `==?` is the mathematical equality operator.
// ,ZORK-NUMBER means to refer to the global ZORK-NUMBER.
...
// Synonym tables create lists of words that are equivalent to the parser.
//
// This includes various prepositions, abbreviated versions of commands, etc.
...
// Syntax tables map verbs to a verb function (so, for instance, VERBOSE will
// execute the V-VERBOSE function).

// These commands are mostly out-of-character commands that change how the game
// functions or print information about the game.
